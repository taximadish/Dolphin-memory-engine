#include "AreaAffinity.h"

AreaAffinity::AreaAffinity(bool serverMode)
{
  if (serverMode)
  {
	m_hostValue = "";
  ...
  else
  {
    ...
    for (int i = 0; i < NUM_AFFINITIES; i++)
    {
      m_AreaAffinityWatches.push_back(new MemWatchEntry(
		  "Area Affinity", BASE_AFFINITIES_ADDR + (i * AFFINITY_SIZE), Common::MemType::type_halfword
	  ));
	}
  }
}

std::string AreaAffinity::Name()
{
  return "AreaAffinity";
}
...
std::string AreaAffinity::setValue(std::string value)
{
  ...
  for (int i = 0; i < NUM_AFFINITIES; i++)
  {
	...
	m_AreaAffinityWatches[i]->writeMemoryFromString(std::to_string(newAffinity));
  }

  return value;
}

std::string AreaAffinity::hostGetValue()
{
  return m_hostValue;
}

std::string AreaAffinity::getUpdate(std::string hostVal)
{
  ...
  for (int i = 0; i < NUM_AFFINITIES; i++)
  {
    ...
    int myAffinity = atoi(m_AreaAffinityWatches[i]->getStringFromMemory().c_str());
...
void AreaAffinity::hostHandleUpdate(int id, std::string updateString)
{
...
std::vector<std::string> AreaAffinity::customSplit(std::string s, std::string delim)
{
